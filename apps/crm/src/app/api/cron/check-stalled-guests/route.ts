 ...
 * app.
 *
 * All the actual logic — find-or-create both persistent campaigns, pull
 * candidates, dedup, draft, best-effort push to apps/telegram-router — lives
 * in ./../../../../lib/campaigns.ts's runCheckStalledGuests, so it's
 * testable without a real Supabase connection. This route is just the
 * auth + wiring + error-to-500 wrapper around it.
 *
 * Returns `{ seasonal_nudge_drafted, stalled_link_nudge_drafted,
 * skipped_already_nudged }` on success, or `{ error }` with 500 on any
 * unexpected failure (a real Supabase error, not a postCampaignDraft
 * failure — that one's already-swallowed inside runCheckStalledGuests).
 */
