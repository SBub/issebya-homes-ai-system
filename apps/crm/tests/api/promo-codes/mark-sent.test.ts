...
const { POST } = await import("@/app/api/promo-codes/[id]/mark-sent/route.js");

function makeRequest(id: string, apiKey = "test-key"): NextRequest {
  return new NextRequest(`http://localhost:3006/api/promo-codes/${id}/mark-sent`, {
    method: "POST",
...
describe("POST /api/promo-codes/[id]/mark-sent", () => {
  ...
  it("marks an 'issued' row as sent, setting status and sent_at", async () => {
    ...
    expect(updateMock).toHaveBeenCalledWith({ status: "sent", sent_at: expect.any(String) });
  ...
  it.each(["sent", "rejected", "expired"])(
    ...
    async (currentStatus) => {
      ...
      expect(json).toEqual({ error: `Cannot mark sent from status '${currentStatus}'` });
      expect(updateMock).not.toHaveBeenCalled();
    },
  );
});
