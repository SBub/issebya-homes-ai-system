...
- **Only the active room is mounted, and a per-room `key` on the tab panel is what makes that true.** Keeping the inactive one hidden would duplicate every `aria-label` on the page and leave locators ambiguous for tests and screen readers. Both rooms' nodes are the same component type at the same position in the tree, so without the `key` React reconciles them as one instance and updates its props instead of remounting - and `BookingClient` seeds its blocked dates, its selected check-in/check-out and its expanded flag from props on mount only, so the guest kept seeing room 1's availability under the room 2 tab (fixed under issue 79). With the `key`, a date selection is dropped on switch, which is also correct: blocked dates differ per room.
