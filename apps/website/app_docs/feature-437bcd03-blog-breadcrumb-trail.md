...
- **Two list items, not three.** The `›` separator is an `aria-hidden` span
  ...
  right-pointing angle quotation mark, A house for rest between the Sintra forest and the Atlantic".
...
3. The line `blog › A house for rest between the Sintra forest and the Atlantic` sits directly above the post
